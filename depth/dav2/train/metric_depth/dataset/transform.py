...
class Resize(object):
    ...
    def __call__(self, sample):
        ...
        if self.__resize_target:
            ...
            if "psuedo_depth" in sample:
                sample["psuedo_depth"] = cv2.resize(
                    sample["psuedo_depth"], (width, height), interpolation=cv2.INTER_NEAREST
                )
...
class PrepareForNet(object):
    ...
    def __call__(self, sample):
        ...
        if "psuedo_depth" in sample:
            depth = sample["psuedo_depth"].astype(np.float32)
            sample["psuedo_depth"] = np.ascontiguousarray(depth)
...
class Crop(object):
    ...
    def __call__(self, sample):
        ...
        if "psuedo_depth" in sample:
            sample["psuedo_depth"] = sample["psuedo_depth"][h_start: h_end, w_start: w_end]
